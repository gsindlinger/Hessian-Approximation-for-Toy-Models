...
class VectorMetric(Enum):
    ...
    def compute(self, v_gt: jnp.ndarray, v_approx: jnp.ndarray, x: jnp.ndarray | None):
        ...
        match self:
            ...
            case VectorMetric.SIGN_AGREEMENT:
                same_sign = jnp.mean(jnp.sign(v_gt) == jnp.sign(v_approx))
                return same_sign
    ...
    def compute_batched(
        ...
        if v_gt.ndim == 1:
            return float(self.compute(v_gt, v_approx, x))
