...
# ===========================================================================
# Pure-function tests: load_attribution_scores
# ===========================================================================


class TestLoadAttributionScores:
    def test_round_trip_single_dir(self, tmp_path):
        n_test, n_train = 4, 7
        a = np.random.randn(n_test, n_train)
        b = np.random.randn(n_test, n_train)
        d = tmp_path / "attrs"
        d.mkdir()
        np.save(str(d / "ekfac.npy"), a)
        np.save(str(d / "gnh.npy"), b)

        out = load_attribution_scores(str(d), n_test, n_train)
        assert set(out.keys()) == {"ekfac", "gnh"}
        np.testing.assert_array_equal(out["ekfac"], a)
        np.testing.assert_array_equal(out["gnh"], b)

    def test_round_trip_multiple_dirs_merge(self, tmp_path):
        n_test, n_train = 4, 7
        a = np.random.randn(n_test, n_train)
        b = np.random.randn(n_test, n_train)
        d1 = tmp_path / "src1"
        ...
        d1.mkdir()
        d2.mkdir()
        np.save(str(d1 / "ekfac.npy"), a)
        np.save(str(d2 / "gnh.npy"), b)

        out = load_attribution_scores([str(d1), str(d2)], n_test, n_train)
        assert set(out.keys()) == {"ekfac", "gnh"}
        np.testing.assert_array_equal(out["ekfac"], a)
        np.testing.assert_array_equal(out["gnh"], b)

    def test_duplicate_method_across_dirs_raises(self, tmp_path):
        n_test, n_train = 3, 5
        d1 = tmp_path / "src1"
        d2 = tmp_path / "src2"
        d1.mkdir()
        d2.mkdir()
        np.save(str(d1 / "ekfac.npy"), np.random.randn(n_test, n_train))
        np.save(str(d2 / "ekfac.npy"), np.random.randn(n_test, n_train))
        with pytest.raises(ValueError, match="Duplicate"):
            load_attribution_scores([str(d1), str(d2)], n_test, n_train)

    def test_missing_dir_raises(self, tmp_path):
        with pytest.raises(FileNotFoundError):
            load_attribution_scores(str(tmp_path / "missing"), 3, 5)

    def test_shape_mismatch_raises(self, tmp_path):
        d = tmp_path / "attrs"
        d.mkdir()
        np.save(str(d / "ekfac.npy"), np.random.randn(3, 5))
        with pytest.raises(ValueError, match="shape"):
            load_attribution_scores(str(d), 4, 5)

    def test_empty_dir_raises(self, tmp_path):
        d = tmp_path / "empty"
        d.mkdir()
        with pytest.raises(ValueError, match="No attribution matrices"):
            load_attribution_scores(str(d), 3, 5)
