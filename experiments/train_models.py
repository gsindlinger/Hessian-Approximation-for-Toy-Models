...
def select_best_models(
    ...
    """
    Select the best model for each architecture/structure group.
    ...
    Args:
        ...
        minimize: If True, select model with minimum metric value

    Returns:
        List of best models, one per group
    """
    ...
    for group_key, group_results in groups.items():
        ...
        logger.info(
            f"[SELECTION] Best for {group_key}: "
            f"{best['model_config'].get_model_display_name()} "
            f"(lr={best['model_config'].training.learning_rate:.2e}, "
            f"wd={best['model_config'].training.weight_decay:.2e}) "
            f"with {selection_metric}={best[selection_metric]:.6f}"
        )
...
def main(cfg: DictConfig):
    ...
    # Prepare best models with proper directory paths for Hessian analysis
    best_models_for_hessian = []
    ...
    logger.info(f"Best models YAML saved to: {best_models_yaml}")
    ...
    logger.info(f"Best models (YAML):  {best_models_yaml}")

    print(f"BEST_MODELS_YAML={best_models_yaml}")
