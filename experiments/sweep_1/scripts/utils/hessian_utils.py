...
from src.utils.manifest import ModelEntry, load_manifest
...
def analyze_single_model(
    ...
    for approx in comp_config.approximators:
        if approx == HessianApproximator.EXACT:
            continue  # Exact only as reference
    ...
    for reference in comp_config.comparison_references:
        ...
        # Depending on reference computer, create computer for reference and select comparison computers
        if reference == ComparisonReference.EXACT:
            ref_computer = HessianComputer(model_ctx)
            comparison_computers = all_computers
            # Avoid comparing exact against itself
            if HessianApproximator.EXACT.value in comparison_computers:
                del comparison_computers[HessianApproximator.EXACT.value]
        elif reference == ComparisonReference.GNH:
            ref_computer = GNHComputer(model_ctx)
            # For GNH comparison avoid comparing against any Hessian (HessianComputer or BlockHessianComputer)
            # Also avoid comparing against itself
            comparison_computers = {
                name: comp
                for name, comp in all_computers.items()
                if name
                not in {
                    HessianApproximator.EXACT.value,
                    HessianApproximator.BLOCK_HESSIAN.value,
                    HessianApproximator.GNH.value,
                }
            }
        else:
            NotImplementedError(f"Reference {reference} not implemented")
    ...
    all_results = {
        "metadata": {
            ...
            "damping_config": {
                ...
                "value": hessian_config.damping_config.get_damping_value(),
            },
            "computation_config": {
                "approximators": [
