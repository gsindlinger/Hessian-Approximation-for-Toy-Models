        ...
        for model_type, model, experiment_hidden_layers in models:
            ...
            for lr in learning_rates:
                for wd in weight_decays:
                    ...
                    model_name = f"{model_type.lower()}_{cfg_hash}_lr{lr}_wd{wd}"
            ...
            collector_dir = f"experiments/sweep_1/data/collector/{best_model_name}/"
            ...
            ekfac = EKFACApproximator(collector_dir, collector_dir)
            ekfac.build(
                Config(
                    digits_dataset_path,
            ...
            damping = max(float(ekfac_data.mean_eigenvalues_aggregated * 0.1), 0.01)
            ...
            # Phase 1: GNH
            # -----------------------------------------------------------------
            logger.info("[HESSIAN] Phase 1: GNH reference")
            ...
            # # -----------------------------------------------------------------
            # # Phase 2: exact Hessian
            # # -----------------------------------------------------------------
            # logger.info("[HESSIAN] Phase 2: exact Hessian reference")
            ...
            # del exact_hessian, H_hvp, H_ihvp, ekfac_data, grads_1, grads_2
